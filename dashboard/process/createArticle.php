   ...
   if ($_FILES['file']['name'] != '' && $title != '') { // is image file avaible?
      ...
      if (in_array($imageFileType, $extensions_arr)) {
         ...
         if (move_uploaded_file($_FILES['file']['tmp_name'], $target_dir . $image)) {
            ...
            $stmt = $connect->prepare("INSERT INTO article(title, img, category_id, user_id, content)
               VALUES(?, ?, ?, ?, ?)");
      ...
      $stmt = $connect->prepare("INSERT INTO article(title, category_id, user_id, content)
               VALUES(?, ?, ?, ?)");
